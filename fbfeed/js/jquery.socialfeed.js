						...
						unifyPostData: function(element) {
							...
							if (options.show_media === true) {
								if (element.entities.media && element.entities.media.length > 0) {
									...
									if (image_url) {
										post.attachment = '<div class="attachment" style="background: url(' + image_url + ');" />';
									}
								}
							}
							return post;
				...
				facebook: {
					...
					utility: {
						prepareAttachment: function(element) {
							...
							return '<div class="attachment" style="background: url(' + image_url + '); background-size: cover;" />';
						},
						...
						unifyPostData: function(element) {
							...
							post.author_name = (element.name) ? element.name : '';
                            post.name = (element.name) ? element.name : '';
                            post.caption = (element.caption) ? element.caption : '';
                            post.icon = element.icon || "";
							post.message = ''; //removes the associated action text e.g. "group y shared a link" is ''.
